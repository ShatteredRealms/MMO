...
TMap<FString, FString> UCommonUserSubsystem::CreateAuthMetaData(int32 PlayerIndex) const
{
	TMap<FString, FString> Out;
	IOnlineIdentity* OnlineIdentity = GetOnlineIdentity();
	if (OnlineIdentity)
	{
		const FString AuthToken = OnlineIdentity->GetAuthToken(PlayerIndex);
		Out.Add("authorization", FString::Printf(TEXT("Bearer %s"), *AuthToken));
	}

	return Out;
}

int32 UCommonUserSubsystem::GetPlayerIndexFromAuthToken(FString AuthToken) const
{
	int32 Index = -1;
	IOnlineIdentity* OnlineIdentity = GetOnlineIdentity();
	if (OnlineIdentity)
	{
		for (const auto& User : OnlineIdentity->GetAllUserAccounts())
		{
		}
	}
...
FString UCommonUserSubsystem::GetUserId(int32 PlayerIndex)
{
	FUniqueNetIdRepl UniqueNetId = GetLocalUserNetId(GetPlatformUserIdForIndex(0));
	FString UserId = UniqueNetId.ToString();
	UserId.RemoveFromStart("SRO:");
	return UserId;
}
