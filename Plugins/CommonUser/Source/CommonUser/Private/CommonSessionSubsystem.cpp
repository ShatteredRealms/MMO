...
void UCommonSessionSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	if (!TLM)
	{
		UE_LOG(LogCommonSession, Error, TEXT("Unable to create TLM"));
		return;
	}
	...
	{
		UE_LOG(LogCommonSession, Error, TEXT("Unable to create connection service"));
		return;
	}

	ConnectionServiceClient = ConnectionService->MakeClient();
	ConnectionServiceClient->OnConnectGameServerResponse.AddUniqueDynamic(this, &ThisClass::OnConnectResponseReceived);
	ConnectionServiceClient->OnVerifyConnectResponse.AddUniqueDynamic(this, &ThisClass::OnVerifyConnectResponseReceived);
...
FString UCommonSessionSubsystem::FindServer()
{
	IOnlineIdentityPtr OnlineIdentityPtr = Online::GetIdentityInterface(GetWorld());
	UCommonCharacterSubsystem* CharacterSubsystem = GetGameInstance()->GetSubsystem<UCommonCharacterSubsystem>();
	...
	const FGrpcSroCharacterCharacter CurrentCharacter = CharacterSubsystem->GetCurrentCharacter();
	if (CurrentCharacter.Id.IsEmpty())
	{
	...
	Request.Id = CurrentCharacter.Id;
	TMap<FString, FString> MetaData = UTurboLinkGrpcUtilities::CreateAuthMetaData(OnlineIdentityPtr->GetAuthToken(0));
	ConnectionServiceClient->ConnectGameServer(Handle, Request, MetaData);
	return TEXT("");
}

void UCommonSessionSubsystem::VerifyConnection(const FUniqueNetIdRepl& UniqueId, FString ConnectionId)
{
	bool bProcessed = false;
#if WITH_SERVER_CODE
	if (bIsDedicatedServer)
	{
		IOnlineSessionPtr OnlineSessionPtr = Online::GetSessionInterface(GetWorld(), SRO_SUBSYSTEM);
		IOnlineIdentityPtr OnlineIdentityPtr = Online::GetIdentityInterface(GetWorld(), SRO_SUBSYSTEM);
		FNamedOnlineSession* NamedOnlineSession = OnlineSessionPtr->GetNamedSession(SRO_SUBSYSTEM);
		if (NamedOnlineSession)
		{
			bProcessed = true;
			
			const auto Handle = ConnectionServiceClient->InitVerifyConnect();
			VerifyConnectCachedUserIds.Add(Handle, FVerifyConnectCache{ UniqueId.ToString(), UniqueId});
			
			FGrpcSroGameserverVerifyConnectRequest Request;
			Request.ConnectionId = ConnectionId;
			Request.ServerName = NamedOnlineSession->SessionName.ToString();
			TMap<FString, FString> MetaData = UTurboLinkGrpcUtilities::CreateAuthMetaData(OnlineIdentityPtr->GetAuthToken(0));
	...
	if (!bProcessed)
	{
		OnVerifyConnectCompleted.Broadcast(UniqueId, FGrpcSroTargetId{}, false);
	}
}

void UCommonSessionSubsystem::OnConnectResponseReceived(FGrpcContextHandle Handle,
                                                        const FGrpcResult& GrpcResult, const FGrpcSroGameserverConnectGameServerResponse& Response)
{
	OnConnectCompleted.Broadcast(Response, GrpcResult.Message);
	K2_OnConnectCompleted.Broadcast(Response, GrpcResult.Message);
}

void UCommonSessionSubsystem::OnVerifyConnectResponseReceived(FGrpcContextHandle Handle, const FGrpcResult& GrpcResult,
	const FGrpcSroTargetId& Response)
{
	FVerifyConnectCache Cache = VerifyConnectCachedUserIds.FindAndRemoveChecked(Handle);
	OnVerifyConnectCompleted.Broadcast(Cache.UniqueId, Response, GrpcResult.Code == EGrpcResultCode::Ok);
}
