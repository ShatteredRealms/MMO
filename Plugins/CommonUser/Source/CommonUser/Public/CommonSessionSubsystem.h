...
struct FVerifyConnectCache
{
	...
	// 	
	// }
	// FVerifyConnectCache(FUniqueNetIdPtr InUniqueId)
	// : UniqueId(InUniqueId),
	//   UserId(InUniqueId->ToString())
	// {
	// 	
	// }
...
DECLARE_MULTICAST_DELEGATE_ThreeParams(FCommonSessionOnVerifyConnectCompleted, const FUniqueNetIdRepl& /* UniqueId */, const FGrpcSroTargetId& /* Character Id */, bool /* bWasSuccessful */);
DECLARE_MULTICAST_DELEGATE_TwoParams(FCommonSessionOnConnectCompleted, const FGrpcSroGameserverConnectGameServerResponse /* ConnectResponse */, FString /* ErrorMessage */);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FCommonSessionOnConnectCompleted_Dynamic, const FGrpcSroGameserverConnectGameServerResponse, ConnectResponse , FString, ErrorMessage);
...
public:
	/** Native Delegate called when finished verifying a connection request */
	FCommonSessionOnVerifyConnectCompleted OnVerifyConnectCompleted;

	FCommonSessionOnConnectCompleted OnConnectCompleted;
	
	UPROPERTY(BlueprintAssignable, Category="CommonSession | Connect", meta=(DisplayName ="On Connect Completed"))
	FCommonSessionOnConnectCompleted_Dynamic K2_OnConnectCompleted;

	void VerifyConnection(const FUniqueNetIdRepl& UniqueId, FString ConnectionId);
	
	UFUNCTION()
	void OnConnectResponseReceived(FGrpcContextHandle Handle, const FGrpcResult& GrpcResult, const FGrpcSroGameserverConnectGameServerResponse& Response);
	
	UFUNCTION()
	void OnVerifyConnectResponseReceived(FGrpcContextHandle Handle, const FGrpcResult& GrpcResult, const FGrpcSroTargetId& Response);
	
private:
	/** Temporarily cache the user UniqueNetId for a outgoing connection verifiaction */
	TMap<FGrpcContextHandle, FVerifyConnectCache> VerifyConnectCachedUserIds;
