...
static bool CanShowSROLogin()
{
	return true;
}

static void LoginSRO_Clicked()
{
	FString LoginUrl = FString::Printf(
		TEXT("http://localhost:8080/realms/default/protocol/openid-connect/auth?client_id=%s&response_type=code&scope=openid profile&redirect_uri=%s"),
		*FString(TEXT("sro-gameserver")),
		*FString(TEXT("http://ue5editor.local")));
	FPlatformProcess::LaunchURL(*LoginUrl, nullptr, nullptr);
}

static TSharedRef<SWidget> GetSROLoginDropdown()
{
	FMenuBuilder MenuBuilder(true, nullptr);
	
		MenuBuilder.AddMenuEntry(
			LOCTEXT("SROLoginLabel", "Login"),
			LOCTEXT("CommonPathDescription", "Opens login url"),
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateStatic(&LoginSRO_Clicked),
				FCanExecuteAction::CreateStatic(&CanShowSROLogin),
				FIsActionChecked(),
				FIsActionButtonVisible::CreateStatic(&CanShowSROLogin)
			)
		);
...
static void RegisterGameEditorMenus()
{
	...
	FToolMenuEntry SROLogin = FToolMenuEntry::InitToolBarButton(
		"SROLogin",
		FUIAction(
			FExecuteAction::CreateStatic(&LoginSRO_Clicked),
			FCanExecuteAction::CreateStatic(&CanShowSROLogin),
			FIsActionChecked(),
			FIsActionButtonVisible::CreateStatic(&CanShowSROLogin)),
		LOCTEXT("SROLogin_Label", "Login SRO"),
		LOCTEXT("SROLogin_ToolTip", "Login in editor to automatically login when starting game"),
		FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.Level")
	);
	CommonMapEntry.StyleNameOverride = "CalloutToolbar";
	Section.AddEntry(SROLogin);
}
