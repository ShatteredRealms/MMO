...
#include "Private/OnlineSessionSRO.h"
...
{
	const auto TLM = UTurboLinkGrpcUtilities::GetTurboLinkGrpcManager(GetWorld());
	if (!TLM)
	{
		UE_LOG(LogLyra, Error, TEXT("Failed to get TurboLinkGrpcManager"));
		return;
	}

	const auto CharacterService = Cast<UCharacterService>(TLM->MakeService("CharacterService"));
	if (!CharacterService)
	{
		UE_LOG(LogLyra, Error, TEXT("Failed to get CharacterService"));
		return;
	}

	CharacterServiceClient = CharacterService->MakeClient();
	CharacterServiceClient->OnGetCharacterResponse.AddUniqueDynamic(this, &ALyraGameMode::OnGetCharacterResponse);
	
	UCommonSessionSubsystem* CSS = GetGameInstance()->GetSubsystem<UCommonSessionSubsystem>();
	CSS->OnVerifyConnectCompleted.AddUObject(this, &ALyraGameMode::OnVerifyConnection);
...
void ALyraGameMode::OnGetCharacterResponse(FGrpcContextHandle Handle, const FGrpcResult& GrpcResult, const FGrpcSroCharacterCharacter& Response)
{
	FString ErrorMessage;
	const auto PendingConnection = PendingConnections.Find(Response.Id);
	if (PendingConnection)
	{
		CachedCharacterData.Add(*PendingConnection, Response);
		FGameModeEvents::GameModePreLoginEvent.Broadcast(this, *PendingConnection, ErrorMessage);
	}
	else
	{
		UE_LOG(LogLyra, Error, TEXT("Failed to find pending connection for character %s"), *Response.Id);
	}
}

void ALyraGameMode::OnVerifyConnection(const FUniqueNetIdRepl& UniqueId, const FGrpcSroTargetId& TargetId, bool bWasSuccessful)
{FString ErrorMessage;
 	if (bWasSuccessful)
 	{
 		UE_LOG(LogLyra, Log, TEXT("Successfully verified connection for %s"), *UniqueId->ToDebugString());
 		PendingConnections.Add(TargetId.Id, UniqueId);
 		IOnlineIdentityPtr OnlineIdentityPtr = Online::GetIdentityInterface(SRO_SUBSYSTEM);
 		if (OnlineIdentityPtr.IsValid())
 		{
 			UE_LOG(LogLyra, Verbose, TEXT("Looking up character details for %s"), *TargetId.Id);
 			const auto Handle = CharacterServiceClient->InitGetCharacter();
 			auto MD = UTurboLinkGrpcUtilities::CreateAuthMetaData(OnlineIdentityPtr->GetAuthToken(0));
 			FGrpcSroCharacterGetCharacterRequest Request;
 			Request.Id = TargetId.Id;
 			CharacterServiceClient->GetCharacter(Handle, Request, MD);
 		}
 		else
 		{
 			ErrorMessage = TEXT("failed to get online identity");
 		}
 	}
 	else
 	{
 		ErrorMessage = TEXT("failed to verify connection");
 	}
 
 	const auto OnComplete = PendingVerifications.Find(UniqueId);
 	if (OnComplete)
 	{
...
void ALyraGameMode::OnMatchAssignmentGiven(FPrimaryAssetId ExperienceId, const FString& ExperienceIdSource)
{
	if (ExperienceId.IsValid())
	{
		...
		// @TODO(wil): Is this needed here anymore?
...
void ALyraGameMode::HandleStartingNewPlayer_Implementation(APlayerController* NewPlayer)
{
	...
	if (IsExperienceLoaded())
	{
		...
		UE_LOG(LogLyra, Log, TEXT("HandleStartingNewPlayer_Implementation: Experience is already loaded, starting player %s"), *GetNameSafe(NewPlayer));
	}
}
...
void ALyraGameMode::PreLoginAsync(const FString& Options, const FString& Address, const FUniqueNetIdRepl& UniqueId,
	const FOnPreLoginCompleteDelegate& OnComplete)
{
	FString ErrorMessage;
	if (UniqueId == nullptr || !UniqueId->IsValid())
	{
		ErrorMessage = TEXT("must be logged in");
	}
	else
	{
	...
	{
		IOnlineSessionPtr OSS = Online::GetSessionInterface(SRO_SUBSYSTEM);
		if (OSS->IsPlayerInSession(NAME_GameSession, *UniqueId))
		{
			ErrorMessage = TEXT("Player already connected");
		}
	}

	if (ErrorMessage.IsEmpty())
	{
		FString ConnectionId = UGameplayStatics::ParseOption(Options, TEXT("cid"));
		if (ConnectionId.IsEmpty())
		{
			ErrorMessage = TEXT("missing cid");
		}
		else
		{
			UCommonSessionSubsystem* CSS = GetGameInstance()->GetSubsystem<UCommonSessionSubsystem>();
			PendingVerifications.Add(UniqueId, OnComplete);
			CSS->VerifyConnection(UniqueId, ConnectionId);
			return;
		}
	}
...
void ALyraGameMode::PostLogin(APlayerController* NewPlayer)
{
	...
	{
		IOnlineSubsystem* const OSS = Online::GetSubsystem(GetWorld());
		check(OSS);
		IOnlineSessionPtr Session = OSS->GetSessionInterface();
		check(Session);

		//@TODO(wil): Handle registering the player with the session in the correct spot. Set the session name
		// to the session name of the session we are hosting prior to joining.
		// Session->RegisterPlayer(FName(), *NewPlayer->GetNetConnection()->PlayerId, false);
		// NewPlayer->PlayerState->SessionName = Session->GetNamedSession(FName())->SessionName;
